...
class MasterDataRepository {
  ...
  // العملات
  Future<int> insertCurrency(Map<String, dynamic> data) => db.into(db.currencies).insert(CurrenciesCompanion(
  ...
  Future<List<Map<String, dynamic>>> getAllCurrencies() async {
    ...
    return rows.map((r) => {'id': r.id, 'code': r.code, 'name': r.name, 'exchange_rate': r.exchangeRate, 'is_default': r.isDefault}).toList();
  }

  Future<int> updateCurrency(int id, String code, String name, double exchangeRate, {bool? isDefault}) async {
    return (db.update(db.currencies)..where((t) => t.id.equals(id))).write(CurrenciesCompanion(
      code: Value(code),
      name: Value(name),
  ...
  Future<int> setDefaultCurrency(int id) async {
    ...
    await (db.update(db.currencies)..where((t) => t.isDefault.equals(true))).write(const CurrenciesCompanion(isDefault: Value(false)));
    // تعيين الجديد
    return (db.update(db.currencies)..where((t) => t.id.equals(id))).write(const CurrenciesCompanion(isDefault: Value(true)));
  }

  Future<Map<String, dynamic>?> getDefaultCurrency() async {
    final row = await (db.select(db.currencies)..where((t) => t.isDefault.equals(true))).getSingleOrNull();
    if (row != null) return {'id': row.id, 'code': row.code, 'name': row.name, 'exchange_rate': row.exchangeRate};
    return null;
  }
  ...
  Future<List<Map<String, dynamic>>> getAllAccounts() async {
    ...
    return rows.map((r) => {'id': r.id, 'number': r.number, 'name_ar': r.nameAr, 'name_en': r.nameEn, 'type': r.type, 'nature': r.nature, 'parent_id': r.parentId, 'level': r.level}).toList();
  }
  Future<int> updateAccount(int id, Map<String, dynamic> data) async => (db.update(db.accounts)..where((t) => t.id.equals(id))).write(AccountsCompanion(nameAr: Value(data['name_ar']), nameEn: Value(data['name_en'])));
  Future<int> deleteAccount(int id) async => (db.delete(db.accounts)..where((t) => t.id.equals(id))).go();
  ...
  Future<int> updateCustomer(int id, Map<String, dynamic> data) async => (db.update(db.customers)..where((t) => t.id.equals(id))).write(CustomersCompanion(name: Value(data['name']), phone: Value(data['phone']), address: Value(data['address'])));
  ...
  Future<int> updateSupplier(int id, Map<String, dynamic> data) async => (db.update(db.suppliers)..where((t) => t.id.equals(id))).write(SuppliersCompanion(name: Value(data['name']), phone: Value(data['phone']), address: Value(data['address'])));
  ...
  Future<int> updateItem(int id, Map<String, dynamic> data) async => (db.update(db.items)..where((t) => t.id.equals(id))).write(ItemsCompanion(name: Value(data['name']), unit: Value(data['unit']), cost: Value(data['cost']?.toString() ?? '0'), price: Value(data['price']?.toString() ?? '0')));
